...
程序说明:通过esp32s3建立一个较为复杂的网页服务器(有两个页面),能通过网页端的操作来控制esp32s3开发板的灯的亮灭
...
*/
void handleUserRequet() {         
  String reqResource = esp32s3_webServe.uri();   // 获取用户请求网址信息
  Serial.print("reqResource: ");
  Serial.println(reqResource);

  bool fileReadOK = handleFileRead(reqResource);   // 通过handleFileRead函数处处理用户访问
...
*/ 
void handleLEDControl(){
  bool ledStatus_a = digitalRead(LED_A);     // 此变量用于储存LED状态
  ledStatus_a == HIGH ? digitalWrite(LED_A, LOW) : digitalWrite(LED_A, HIGH);  // 点亮或者熄灭LED_A  
     
  bool ledStatus_b = digitalRead(LED_B);     // 此变量用于储存LED状态
  ledStatus_b == HIGH ? digitalWrite(LED_B, LOW) : digitalWrite(LED_B, HIGH);  // 点亮或者熄灭LED_B

  //发送请求以及响应状态码,这两句代码功能:浏览器发送跳转到 "/LED.html" 的请求向服务器,
  //现象是:当按下对应的按键时浏览器就会刷新一次
  esp32s3_webServe.sendHeader("Location", "/LED.html");       
  esp32s3_webServe.send(303);  
}
